...
class PathFindingEnvWithMap(gym.Env):
    ...
    def _compute_flow_field(self, goal_i, goal_j):
        ...
        heapq.heappush(pq, (0, goal_i, goal_j))
        ...
        for i in range(self.H):
            for j in range(self.W):
                if dist[i,j] == INF or self.grid[i,j] > 0.5:
                    continue
                best_d = dist[i,j]
                best_vec = None
                for di, dj in neighs:
                    ni, nj = i+di, j+dj
                    if (0 <= ni < self.H and 0 <= nj < self.W and
                        dist[ni,nj] < best_d):
                        best_d = dist[ni,nj]
                        best_vec = np.array([dj, di], np.float32)  # x=col, y=row
                if best_vec is not None:
                    norm = np.linalg.norm(best_vec)
                    flow[i,j] = best_vec / (norm + 1e-8)
        return flow
    ...
    def _get_obs(self):
        ...
        return {"vec_obs": vec_obs, "next_obs": next_obs, "action_mask": mask}, None
